...
class Validation extends \ArrayObject
{
     ...
     * in_array 验证
     * Example
     * $post->add_rules('int', 'in[1,2,3,4]');
     * @param   mixed $str Input value
     * @param   array $inputs Input names to match against
     * @return  bool
     */
    public function in($str, array $inputs)
    {
        return in_array($str, $inputs);
    }

    /**
     * 大于,小于
     * Example 大于0,小于999
     * $post->add_rules('int', 'gtlt[1,999]');
     * @param $str
     * @param array $inputs
     * @return bool
     * @throws BizException
     */
    public function gtlt($str, array $inputs)
    {
        try {
            return $str > $inputs[0] && $str < $inputs[1];
        } catch (\Exception $e) {
            throw new BizException(BizException::SELF_DEFINE, $e->getMessage() . 'add_rule gtlt 代码参数错误');
        }
    }
    ...
    /**
     * 验证日期格式eg 2018-09-23 这种日期格式
     * example: $post->add_rules('created_at', 'mysqlDate');
     * @param $value
     * @return bool
     */
    public function mysqlDate($value)
    {
